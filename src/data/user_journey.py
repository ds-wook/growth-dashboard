...
def filter_starting_step(x: pd.Series, starting_step: str, n_steps: int) -> List[Any]:
    """
    ...
    assert isinstance(
        x, (list, pd.Series)
    ), '"x" should be a python list or pandas series containing event names'
    assert isinstance(
        starting_step, str
    ), '"starting_step" should be a string resembling an event name'
...
def user_journey(
    ...
    flow = (
        events[(events["distinct_id"].isin(valid_ids))]
        .groupby("distinct_id")
        .name.agg(list)
        .to_frame()["name"]
        .apply(
            lambda x: filter_starting_step(
                x, starting_step=starting_step, n_steps=n_steps
            )
        )
        .to_frame()["name"]
        .apply(pd.Series)
    ...
    for col in flow.columns:
        ...
        flow[col].replace(
            to_replace, [str(col + 1) + ": Other"] * len(to_replace), inplace=True
        )

    # count the number of identical journeys up the max step defined
    flow = (
        flow.groupby(list(range(n_steps)))
        .size()
        .to_frame()
...
def sankey_df(
    ...
    source_target_df = source_target_df[
        (~source_target_df["source"].str.contains("End"))
